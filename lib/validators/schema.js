...
module.exports = class SchemaValidator extends GenericValidator {
  ...
  static async validate(params) {
    ...
    const schemaValidator = resolve.validator(params.validator, value, params.key, params.payload)
    const shape = resolve.shape(schemaValidator, value)
    ...
    for (const key of Object.keys(shape)) {
      ...
      const schemaErrors = await Validator.validate({
        payload: value,
        validator: shape[key],
        path: resolve.path(params.path, key, schemaValidator),
        key: resolve.key(key, schemaValidator),
        bulk: params.bulk,
      })
