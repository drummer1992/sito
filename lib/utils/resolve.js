...
exports.validator = async (params, value) => {
  ...
  if (typeof validator === 'function') {
    validator = await validator(value, params.key, params.payload, params.root, params.path)
  }

  if (validator instanceof GenericValidator) {
    params.normalize && validator.normalize()
...
const makeOptional = validate => (...args) => {
  const [value] = args
...
exports.validate = (checkDto, commonCheck) => {
  ...
  let validate = toFunc(checkDto.validate)
