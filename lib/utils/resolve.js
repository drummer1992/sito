...
exports.value = async params => {
  const { key, payload, validator } = params

  const isRoot = isNil(key)

  const value = isRoot ? payload : (payload && payload[key])

  if (validator.mapper) {
    const transformed = await validator.mapper(value, key, payload)

    isRoot && (params.payload = transformed)
    !isRoot && (params.payload[key] = transformed)
