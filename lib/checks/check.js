...
module.exports = class Check {
  ...
  async execute(params) {
    const { key, value, payload, root, path = 'payload' } = params

    if (!await this.validate(value, key, payload, root, path)) {
      const message = await this.message(path, value, key, payload, root)
