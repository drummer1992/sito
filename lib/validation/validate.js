...
const validate = async params => {
  ...
  await validator.checks.execute({ ...params, validator, value })
    .catch(async error => {
      if (validator.mappers.onError.length) {
        const onErrorMappers = validator.mappers.onError.map(mapper => ({
          ...mapper,
          fn: (...args) => mapper.fn(error, ...args),
        }))

        value = await runMappers(value, onErrorMappers, params, isRoot)
      } else if (params.throwFirst) {
        throw error
      } else {
        errors.push(error)
      }
